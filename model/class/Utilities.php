        ...
        public static function validateString(...$listStringsToValidate): bool {
            if (count($listStringsToValidate) > 0) {
                foreach ($listStringsToValidate as $stringToValidate) {
                        if (gettype($stringToValidate[0]) == "string" && $stringToValidate[0] != null && $stringToValidate[0] != "" && (!(preg_match($stringToValidate[1],$stringToValidate[0])) || (strlen($stringToValidate[0]) > $stringToValidate[2] && strlen($stringToValidate[0]) < $stringToValidate[3]) ) ) {
                            return false;
                        }
                }
            }
